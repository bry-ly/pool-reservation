...
import { IconMinus, IconTrendingUp, IconArrowUp, IconChevronUp, IconTrendingDown, IconArrowDown, IconChevronDown } from "@tabler/icons-react";
...
function DeltaIcon({
	...
	if (!resolvedValue || resolvedValue === 0) {
		return shell(
			<IconMinus {...slotProps} className={mergedClassName} {...props} />
		);
	}
	...
	if (resolvedValue > 0) {
		if (variant === "trend") {
			return shell(
				<IconTrendingUp {...slotProps} className={mergedClassName} {...props} />
			);
		}

		if (variant === "arrow") {
			return shell(
				<IconArrowUp {...slotProps} className={mergedClassName} {...props} />
			);
		}

		return shell(
			<IconChevronUp {...slotProps} className={mergedClassName} {...props} />
		);
	}

	if (variant === "trend") {
		return shell(
			<IconTrendingDown {...slotProps} className={mergedClassName} {...props} />
		);
	}

	if (variant === "arrow") {
		return shell(
			<IconArrowDown {...slotProps} className={mergedClassName} {...props} />
		);
	}

	return shell(
		<IconChevronDown {...slotProps} className={mergedClassName} {...props} />
	);
}
